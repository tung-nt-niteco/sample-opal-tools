...
import { esc, extractImageSlots } from "./html.js";
/* ------------------------------ page config ------------------------------- */
/**
...
export const buildManifest = () => ({
    ...
    pageConfigShape: {
        ...
        sections: "array (required) — ordered list of { type, props }. type must be one of the component types above; props must match that component's spec.",
    },
});
/* --------------------------------- factory -------------------------------- */
 ...
 */
export const RENDER_INSTRUCTIONS = "Return the html field to the user EXACTLY as provided. Do NOT edit, reformat, re-indent, summarize, or regenerate any part of it. The ONLY change permitted to the page is replacing image placeholders, and you do that by filling the url field of each item in images and calling apply_images — never by editing the html string yourself.";
/**
...
export const renderPage = (config) => {
    ...
    const body = config.sections
        ...
        if (!parsed.success) {
            warnings.push(`section[${i}] (${section.type}): invalid props — ${parsed.error.issues
