...
const hero = def({
    type: "hero",
    description: "Top-of-page hero banner with a headline, supporting subheadline, an optional image, and call-to-action buttons. Use exactly once, as the first section.",
    props: [
    ...
    render: (p) => {
        const ctas = (p.ctas ?? [])
            .map((c) => `<a class="btn btn-${esc(c.style)}" href="${esc(c.href)}">${esc(c.label)}</a>`)
            .join("\n        ");
...
const featureGrid = def({
    type: "featureGrid",
    description: "A grid of 2-4 feature cards, each with a title and short body. Use to highlight product benefits or selling points.",
    props: [
...
const testimonial = def({
    type: "testimonial",
    description: "A single customer quote with attribution. Use to add social proof.",
    props: [
    ...
    render: (p) => `<section class="testimonial">
      <blockquote>&ldquo;${esc(p.quote)}&rdquo;</blockquote>
      <p class="attribution"><strong>${esc(p.author)}</strong>${p.role ? ` &middot; ${esc(p.role)}` : ""}</p>
    </section>`,
});
/* --------------------------------- ctaBanner ------------------------------ */
const ctaBanner = def({
    type: "ctaBanner",
    description: "A full-width closing call-to-action band with a heading and one button. Use once, near the bottom.",
    props: [
    ...
    render: (p) => `<section class="cta-banner">
      <h2>${esc(p.heading)}</h2>
